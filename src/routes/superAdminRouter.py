...
superAdminRouter = APIRouter(prefix="/admin",dependencies=[Depends(superAdminAuth)])
...
# admin
@superAdminRouter.post("/admin",response_model=ApiResponse[UserBase],tags=["SUPERADMIN/ADMIN"])
...
@superAdminRouter.get("/admin",response_model=ApiResponse[list[UserBase]],tags=["SUPERADMIN/ADMIN"])
async def get_all_admin(session : sessionDepedency = None) :
    return await superAdminService.getAllAdmin(session)

@superAdminRouter.get("/admin/{id_admin}",response_model=ApiResponse[UserBase],tags=["SUPERADMIN/ADMIN"])
...
@superAdminRouter.put("/admin/{id_admin}",response_model=ApiResponse[UserBase],tags=["ADMIN/admin"])
async def update_admin(id_admin : int,admin : UpdateAdminRequest,session : sessionDepedency = None) :
    return await superAdminService.updateAdmin(id_admin,admin,session)

@superAdminRouter.delete("/admin/{id_admin}",response_model=ApiResponse[UserBase],tags=["ADMIN/admin"])
