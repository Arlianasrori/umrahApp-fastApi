...
from ..auth.auth_depends.user.get_user_auth import getUsetAuth
...
async def add_package_rating(rating : AddRatingRequest,user : dict = Depends(getUsetAuth),session : sessionDepedency = None) :
...
async def add_package_rating(id_rating : int, rating : UpdateRatingRequest,user : dict = Depends(getUsetAuth),session : sessionDepedency = None) :
...
async def add_package_rating(id_rating : int, user : dict = Depends(getUsetAuth),session : sessionDepedency = None) :
...
async def add_booking(request : AddBookingRequest,user : dict = Depends(getUsetAuth),session : sessionDepedency = None) :
